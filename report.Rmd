...
output:
  word_document:
    reference_docx: styles/manuscript_style_ref.docx
---
...
table.1 <- read_csv2("Tables/Table_1.csv")
...
```{r Figure_S4, fig.width=3.95, fig.height=5.83, fig.cap="__Figure S4: Microbial distribution according to Bray-Curtis distance.__ a) Principal coordinates analyses on Bray-Curtis distances inferred form 16S rRNA and ITS1 metabarcoding. Environmental samples and crab’s tissues (top side of the panels) were reported separately as distribution obtained with the two markers reported (right side of each panel). Solid-colored points represent different samples whereas white-filled points represent centroids. The variance of the objects along each axis has been reported between squared brackets (from the lowest to the highest) and different sample types were reported using different colors (TKP, To Kwa Peng; SK, Sai Keng; SH, Shui Hau). b) Permutational analysis of variance on ordinations reported in panel a. For each pair of tissues and environmental samples a permutational analysis of variance was performed. R-squared values of significant contrasts were reported for both 16S (upper triangle) and ITS (lower triangle) counts."}
figure.s4
```
...
::: {custom-style="Compact"}
__Table S2: Accuracy and correlation between replicates.__ Sample ID were reported in the first column whereas crab's tissues were reported in the second one. The number of fungal ASVs detected in all tissues was reported in the "N" column. Spearman’s rank correlation coefficient was reported in the $\rho$ column whereas accuracy was reported in the last column of the table. Other columns report values used to calculate the accuracy as reported in the "Fungal amplicon sequence variant evaluation" section.
:::

```{r Table_S2, results='asis'}
table.s2 <- rhos
colnames(table.s2) <- c("Sample ID", "Tissue", "N", "$\\rho$", "TP", "TN", "FP + FN", "Accuracy")
